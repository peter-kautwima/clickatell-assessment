...
def keyless_settings(monkeypatch):
    """Pin the LLM toggle to keyless. The settings singleton is built at
    import time, so a real ANTHROPIC_API_KEY in .env would silently flip the
    whole suite onto the live path. Live-path tests set a fake key explicitly.
    """
